...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            if (jwt != null && jwtTokenProvider.validateToken(jwt)) {
                ...
                // Blocca utenti bannati — risponde 403 senza impostare authentication
                if (!userDetails.isAccountNonLocked()) {
                    logger.warn("🚫 Utente bannato ha tentato di accedere: {}", username);
    ...
    private void sendBannedResponse(HttpServletResponse response) throws IOException {
        ...
        new ObjectMapper().writeValue(response.getOutputStream(),
                Map.of("status", 403, "message", "Il tuo account è stato sospeso. Contatta il supporto."));
    }
}